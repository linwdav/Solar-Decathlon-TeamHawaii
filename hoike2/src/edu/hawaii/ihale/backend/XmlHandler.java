...
/**
 * Handles parsing the IHale System XML documents in order to store the data in the Berkeley
 * repository.
 * 
 * @author Tony Gaskell
...
public class XmlHandler {
   ...
   * <url>http://code.google.com/p/solar-decathlon-teamhawaii/wiki/HouseSystemRestAPI</url> as of
  ...
  public Boolean parseIhaleXml2StateEntry(Document doc) throws IOException,
    ...
    Double systems = (Double) xpath.evaluate("count(//state-data)", doc, XPathConstants.NUMBER);
    ...
    for (int i = 1; i <= systems.intValue(); i++) {
      String systemEnumString =
          (String) xpath.evaluate(stateData + i + "]/@system", doc, XPathConstants.STRING);
      systemEnum = IHaleSystem.valueOf(systemEnumString.toUpperCase(Locale.ENGLISH));
      device = (String) xpath.evaluate(stateData + i + "]/@device", doc, XPathConstants.STRING);
      timestamp =
          Long.parseLong((String) xpath.evaluate(stateData + i + "]/@timestamp", doc,
              XPathConstants.STRING));
      // System.out.println("********************");
      // System.out.println(systemEnum.toString() + " " + device + " " + timestamp);
      Double states =
          (Double) xpath.evaluate("count(//state-data[" + i + "]/state)", doc,
      ...
      for (int j = 1; j <= states.intValue(); j++) {
        stateEnum =
            IHaleState.valueOf((String) xpath.evaluate(stateData + i + "]" + "/state[" + j
                + "]/@key", doc, XPathConstants.STRING));
        tempVal =
            (String) xpath.evaluate(stateData + i + "]" + "/state[" + j + "]/@value", doc,
                XPathConstants.STRING);
        if (stateEnum.getType().equals(Double.class)) {
          ...
          System.err.println("ERROR: Could not parse value: " + tempVal + " for IHaleState: "
              + stateEnum.toString());
        }
        /*
         * System.out.print(stateEnum.toString()); if (roomEnum != null) { System.out.print(" " +
         * roomEnum.toString()); } System.out.println(" " + tempVal);
         */
        // Store to the repository
        repository.store(systemEnum, roomEnum, stateEnum, timestamp, finalVal);
        /*
         * System.err.print("Storing: " + systemEnum.toString() + " "); if (roomEnum != null) {
         * System.err.print(roomEnum.toString() + " "); } System.err.println(stateEnum.toString() +
         * " " + finalVal.toString());
         */
      }
    }
   ...
   * <url>http://code.google.com/p/solar-decathlon-teamhawaii/wiki/HouseSystemRestAPI</url> as of
   * March 15, 2011, and stores it to the IHaleRepository.
   * 
   ...
   * <url>http://www.egauge.net/docs/egauge-xml-api.pdf</url> as of March 15, 2011, and stores it to
   * the IHaleRepository. If something fails inside this method, nothing will be stored inside the
   * repository.
   * 
   * @param doc the Document to be parsed.
   * @return boolean true if successful, false otherwise.
  ...
  public Boolean parseEGaugeXmlToStateEntry(Document doc) throws IOException,
    ...
    for (int i = 1; i < meters.intValue(); i++) {
      ...
      if ("Grid".equals(title)) {
        ...
        tempVal =
            (String) xpath.evaluate(meter + i + "]" + "/energy/text()", doc, XPathConstants.STRING);
      }
      /*
       * else { System.err.println("Unknown meter: " + title + " (This is okay)"); } if (systemEnum
       * != null) { System.out.print(systemEnum.toString() + " "); }
       * 
      ...
      try {
        repository.store(systemEnum, roomEnum, stateEnum, timestamp, finalVal);
        /*
         * System.err.println("Storing: " + systemEnum.toString() + " " + stateEnum.toString() + " "
         * + finalVal.toString());
         */
      }
      catch (Exception e) {
