...
public class AquaPonics extends Header {
    ...
    private static final Label recommendedLevelLabel = new Label(
			"RecommendedLevelLabel", LEVEL_RANGE_START + dash
					+ LEVEL_RANGE_END + " inches");
	...
	public AquaPonics() throws Exception {
		...
		Model<String> tempStatusModel = new Model<String>() {
			...
			public String getObject() {
				...
				long value = SolarDecathlonApplication.getAquaponics()
						.getTemp();
				String status;
				if ((value < TEMPERATURE_RANGE_START 
						&& value > TEMPERATURE_ALERT_RANGE_START) 
						|| (value > TEMPERATURE_RANGE_END 
		...
		Model<String> oxygenStatusModel = new Model<String>() {
			...
			public String getObject() {
				...
				double value = SolarDecathlonApplication.getAquaponics()
						.getOxygen();
				String status;
				if (Math.abs(value - OXYGEN_RANGE_START) 
						< OXYGEN_ACCEPTED_DIFFERENCE
						|| Math.abs(value - OXYGEN_RANGE_END) 
						< OXYGEN_ACCEPTED_DIFFERENCE) {
					status = WARNG_MESSAGE;
				} 
		...
		Model<String> levelStatusModel = new Model<String>() {
			...
			public String getObject() {
				...
				if (Math.abs(value - LEVEL_RANGE_START) < LEVEL_ACCEPTED_DIFFERENCE
						|| Math.abs(value - LEVEL_RANGE_END) 
						< LEVEL_ACCEPTED_DIFFERENCE) {
					status = WARNG_MESSAGE;
				} 
		...
		Model<String> circulationStatusModel = new Model<String>() {
			...
			public String getObject() {
				...
				if (Math.abs(value - CIRCULATION_RANGE_START) 
						< CIRCULATION_ACCEPTED_DIFFERENCE
						|| Math.abs(value - CIRCULATION_RANGE_END) 
						< CIRCULATION_ACCEPTED_DIFFERENCE) {
					status = WARNG_MESSAGE;
				} 
		...
		Model<String> turbidityStatusModel = new Model<String>() {
			...
			public String getObject() {
				...
				if (Math.abs(value - TURBIDITY_RANGE_START) 
						< TURBIDITY_ACCEPTED_DIFFERENCE
						|| Math.abs(value - TURBIDITY_RANGE_END) 
						< TURBIDITY_ACCEPTED_DIFFERENCE) {
					status = WARNG_MESSAGE;
				} 
		...
		Model<String> phColorModel = new Model<String>() {
			...
			public String getObject() {
				double value = SolarDecathlonApplication.getAquaponics()
						.getPH();
				String color;
				if (Math.abs(value - PH_RANGE_START) 
						< PH_ACCEPTED_DIFFERENCE
						|| Math.abs(value - PH_RANGE_END) 
						< PH_ACCEPTED_DIFFERENCE) {
					color = WARNG_BACKGROUND;
				} 
				else if (value < PH_RANGE_START 
						|| value > PH_RANGE_END) {
					color = ALERT_BACKGROUND;
				} 
				else {
		...
		Model<String> ecColorModel = new Model<String>() {
			...
			public String getObject() {
				double value = SolarDecathlonApplication.getAquaponics()
						.getConductivity();
				String color;
				if (Math.abs(value - EC_RANGE_START) 
						< PH_ACCEPTED_DIFFERENCE
						|| Math.abs(value - EC_RANGE_END) 
						< PH_ACCEPTED_DIFFERENCE) {
					color = WARNG_BACKGROUND;
		...
		Model<String> levelColorModel = new Model<String>() {
			...
			public String getObject() {
				int value = SolarDecathlonApplication.getAquaponics()
						.getWaterLevel();
				String color;
				if (Math.abs(value - LEVEL_RANGE_START) 
						< LEVEL_ACCEPTED_DIFFERENCE
						|| Math.abs(value - LEVEL_RANGE_END) 
						< LEVEL_ACCEPTED_DIFFERENCE) {
					color = WARNG_BACKGROUND;
				} 
				else if (value < LEVEL_RANGE_START 
	 ...
	 * 
	 * @param wmc
	 *            web component to display graph in
	 **/
	private void displayDayGraph(WebMarkupContainer wmc) {
		String graphURL;
		DecimalFormat df = new DecimalFormat("#");
		String tempValue = String.valueOf(SolarDecathlonApplication
				.getAquaponics().getTemp());
		String phValue = df.format(SolarDecathlonApplication.getAquaponics()
				.getPH());
		String oxygenValue = df.format(SolarDecathlonApplication
				.getAquaponics().getOxygen());
		graphURL = "http://chart.apis.google.com/chart"
				+ "?chxl=0:|Temp|pH|Oxygen" + "&chxt=x,y" + "&chbh=a,5,15"
				+ "&chs=300x200" + "&cht=bvg" + "&chco=008000,FF9900"
				+ "&chd=t:73,6.8,5|" + tempValue + "," + phValue + ","
				+ oxygenValue + "&chdl=Recommended|Actual" + "&chdlp=t";
