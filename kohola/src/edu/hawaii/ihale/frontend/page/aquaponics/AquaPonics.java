...
public class AquaPonics extends Header {
	...
	private static final Label recommendedTempLabel = new Label(
			"RecommendedTempLabel", TEMPERATURE_RANGE_START + "&deg;F - "
					+ TEMPERATURE_RANGE_END + "&deg;F");

	private static final Label recommendedPHLabel = new Label(
			"RecommendedPHLabel", PH_RANGE_START + dash + PH_RANGE_END);

	private static final Label recommendedOxygenLabel = new Label(
			"RecommendedOxygenLabel", OXYGEN_RANGE_START + dash
					+ OXYGEN_RANGE_END + " mg/l");

	private static final Label recommendedECLabel = new Label(
			"RecommendedECLabel", EC_RANGE_START + dash + EC_RANGE_END
					+ " &micro;s/cm");

	private static final Label recommendedLevelLabel = new Label(
			"RecommendedLevelLabel", LEVEL_RANGE_START + dash + LEVEL_RANGE_END
					+ " inches");

	private static final Label recommendedCirculationLabel = new Label(
			"RecommendedCirculationLabel", CIRCULATION_RANGE_START + dash
					+ CIRCULATION_RANGE_END + " gpm");

	private static final Label recommendedTurbidityLabel = new Label(
			"RecommendedTurbidityLabel", TURBIDITY_RANGE_START + dash
	...
	public AquaPonics() throws Exception {
		...
		((SolarDecathlonSession) getSession()).getHeaderSession().setActiveTab(
				2);
		...
		List<SystemStatusMessage> msgs = SolarDecathlonApplication
				.getMessages().getMessages(IHaleSystem.AQUAPONICS);

		// Create wrapper container for pageable list view
		WebMarkupContainer systemLog = new WebMarkupContainer(
     ...
     add(helpLink);		
		...
		PageableListView<SystemStatusMessage> listView = new PageableListView<SystemStatusMessage>(
			...
			protected void populateItem(ListItem<SystemStatusMessage> item) {
				...
				if (msg.getType() == null) {
					...
					item.add(new Label("AquaponicsMessageContent",
							"No Messages"));
				}
				// Populate data
				else {
					item.add(new Label("AquaponicsTimestamp", new Date(msg
							.getTimestamp()).toString()));
					item.add(new Label("AquaponicsMessageType", msg.getType()
							.toString()));
					item.add(new Label("AquaponicsMessageContent", msg
							.getMessage()));
				}
			}
		};
		...
		Model<String> tempStatusModel = new Model<String>() {
			...
			public String getObject() {
				...
				long value = SolarDecathlonApplication.getAquaponics()
						.getTemp();
				String status;
				...
				} else if (value <= TEMPERATURE_ALERT_RANGE_START
						|| value >= TEMPERATURE_ALERT_RANGE_END) {
		...
		Model<String> temp = new Model<String>() {
			...
			@Override
			public String getObject() {
				return String.valueOf(SolarDecathlonApplication.getAquaponics()
						.getTemp());
			}
		};
		...
		Model<String> pHStatusModel = new Model<String>() {
			...
			public String getObject() {
				// set the text and color according to the value
				double value = SolarDecathlonApplication.getAquaponics()
						.getPH();
				String status;
		...
		Model<String> oxygenStatusModel = new Model<String>() {
			...
			public String getObject() {
				...
				if (Math.abs(value - OXYGEN_RANGE_START) < OXYGEN_ACCEPTED_DIFFERENCE
					...
					status = WARNG_MESSAGE;
				} else if (value < OXYGEN_RANGE_START
						|| value > OXYGEN_RANGE_END) {
					status = ALERT_MESSAGE;
				} else {
					status = NORM_MESSAGE;
				}
		...
		Model<String> levelStatusModel = new Model<String>() {
			...
			public String getObject() {
				// set the text and color according to the value
				int value = SolarDecathlonApplication.getAquaponics()
						.getWaterLevel();
				String status;
		...
		Model<String> circulationStatusModel = new Model<String>() {
			...
			public String getObject() {
				...
				if (Math.abs(value - CIRCULATION_RANGE_START) < CIRCULATION_ACCEPTED_DIFFERENCE
					...
					status = WARNG_MESSAGE;
				} else if (value < CIRCULATION_RANGE_START
						|| value > CIRCULATION_RANGE_END) {
					status = ALERT_MESSAGE;
				} else {
					status = NORM_MESSAGE;
				}
		...
		Label circulationStatusLabel = new Label("CirculationStatus",
				circulationStatusModel);
		...
		Model<String> turbidityStatusModel = new Model<String>() {
			...
			public String getObject() {
				...
				if (Math.abs(value - TURBIDITY_RANGE_START) < TURBIDITY_ACCEPTED_DIFFERENCE
					...
					status = WARNG_MESSAGE;
				} else if (value < TURBIDITY_RANGE_START
						|| value > TURBIDITY_RANGE_END) {
					status = ALERT_MESSAGE;
				} else {
					status = NORM_MESSAGE;
				}
		...
		Label turbidityStatusLabel = new Label("TurbidityStatus",
				turbidityStatusModel);
		...
		circulationColorLabel = new Label("circulationColor",
				circulationColorModel);
		...
		WebMarkupContainerWithAssociatedMarkup tempInnerDiv = new WebMarkupContainerWithAssociatedMarkup(
				"TempInnerDiv");

		WebMarkupContainerWithAssociatedMarkup tempOuterDiv = new WebMarkupContainerWithAssociatedMarkup(
				"TempOuterDiv");
		...
		tempOuterDiv.add(new AbstractBehavior() {
			...
			public void onComponentTag(Component component, ComponentTag tag) {
				tag.put(classTagName, boxTagName);
				tag.put(styleTagName,
						tempColorLabel.getDefaultModelObjectAsString());
			}
		});
		...
		WebMarkupContainerWithAssociatedMarkup phInnerDiv = new WebMarkupContainerWithAssociatedMarkup(
				"PhInnerDiv");

		WebMarkupContainerWithAssociatedMarkup phOuterDiv = new WebMarkupContainerWithAssociatedMarkup(
				"PhOuterDiv");
		...
		phOuterDiv.add(new AbstractBehavior() {
			...
			public void onComponentTag(Component component, ComponentTag tag) {
				tag.put(classTagName, boxTagName);
				tag.put(styleTagName,
						phColorLabel.getDefaultModelObjectAsString());
			}
		});
		...
		WebMarkupContainerWithAssociatedMarkup oxygenInnerDiv = new WebMarkupContainerWithAssociatedMarkup(
				"OxygenInnerDiv");

		WebMarkupContainerWithAssociatedMarkup oxygenOuterDiv = new WebMarkupContainerWithAssociatedMarkup(
				"OxygenOuterDiv");
		...
		oxygenOuterDiv.add(new AbstractBehavior() {
			...
			public void onComponentTag(Component component, ComponentTag tag) {
				tag.put(classTagName, boxTagName);
				tag.put(styleTagName,
						oxygenColorLabel.getDefaultModelObjectAsString());
			}
		});
		...
		// Auto refresh the div
		oxygenOuterDiv.add(new AjaxSelfUpdatingTimerBehavior(Duration
				.seconds(5)) {
			private static final long serialVersionUID = 1L;
		});
		...
		WebMarkupContainerWithAssociatedMarkup ecInnerDiv = new WebMarkupContainerWithAssociatedMarkup(
				"EcInnerDiv");

		WebMarkupContainerWithAssociatedMarkup ecOuterDiv = new WebMarkupContainerWithAssociatedMarkup(
				"EcOuterDiv");
		...
		ecOuterDiv.add(new AbstractBehavior() {
			...
			public void onComponentTag(Component component, ComponentTag tag) {
				tag.put(classTagName, boxTagName);
				tag.put(styleTagName,
						ecColorLabel.getDefaultModelObjectAsString());
			}
		});
		...
		WebMarkupContainerWithAssociatedMarkup levelInnerDiv = new WebMarkupContainerWithAssociatedMarkup(
				"LevelInnerDiv");

		WebMarkupContainerWithAssociatedMarkup levelOuterDiv = new WebMarkupContainerWithAssociatedMarkup(
				"LevelOuterDiv");
		...
		levelOuterDiv.add(new AbstractBehavior() {
			...
			public void onComponentTag(Component component, ComponentTag tag) {
				tag.put(classTagName, boxTagName);
				tag.put(styleTagName,
						levelColorLabel.getDefaultModelObjectAsString());
			}
		});
		...
		WebMarkupContainerWithAssociatedMarkup circulationInnerDiv = new WebMarkupContainerWithAssociatedMarkup(
				"CirculationInnerDiv");

		WebMarkupContainerWithAssociatedMarkup circulationOuterDiv = new WebMarkupContainerWithAssociatedMarkup(
				"CirculationOuterDiv");
		...
		circulationOuterDiv.add(new AbstractBehavior() {
			...
			public void onComponentTag(Component component, ComponentTag tag) {
				tag.put(classTagName, boxTagName);
				tag.put(styleTagName,
						circulationColorLabel.getDefaultModelObjectAsString());
			}
		});
		...
		// Auto refresh the div
		circulationOuterDiv.add(new AjaxSelfUpdatingTimerBehavior(Duration
				.seconds(5)) {
			private static final long serialVersionUID = 1L;
		});
		...
		WebMarkupContainerWithAssociatedMarkup turbidityInnerDiv = new WebMarkupContainerWithAssociatedMarkup(
				"TurbidityInnerDiv");

		WebMarkupContainerWithAssociatedMarkup turbidityOuterDiv = new WebMarkupContainerWithAssociatedMarkup(
				"TurbidityOuterDiv");
		...
		turbidityOuterDiv.add(new AbstractBehavior() {
			...
			public void onComponentTag(Component component, ComponentTag tag) {
				tag.put(classTagName, boxTagName);
				tag.put(styleTagName,
						turbidityColorLabel.getDefaultModelObjectAsString());
			}
		});
		...
		// Auto refresh the div
		turbidityOuterDiv.add(new AjaxSelfUpdatingTimerBehavior(Duration
				.seconds(5)) {
			private static final long serialVersionUID = 1L;
		});
		...
		waterTemp = new TextField<String>("waterTemperature",
				new Model<String>(setTemp + ""));
		...
		waterTemp.add(new AjaxFormComponentUpdatingBehavior("onchange") {
			...
			protected void onUpdate(AjaxRequestTarget target) {
				...
				SolarDecathlonApplication.getBackend().doCommand(system, null,
		...
		// Add the control for the water temp slider
		waterPh = new TextField<String>("waterPH",
				new Model<String>(setPh + ""));
		...
		// Add the control for the water temp slider
		waterLevel = new TextField<String>("waterLEVEL", new Model<String>(
				setLevel + ""));
		...
		// Add the control for the water temp slider
		waterNutrients = new TextField<String>("waterNUTRIENTS",
				new Model<String>(setNutrients + ""));
		...
		waterNutrients.add(new AjaxFormComponentUpdatingBehavior("onchange") {
			...
			protected void onUpdate(AjaxRequestTarget target) {
				...
				SolarDecathlonApplication.getBackend().doCommand(system, null,
						command, newNutrients);
			}
		});

		waterNutrients.add(new AjaxSelfUpdatingTimerBehavior(Duration
				.seconds(10)) {
			private static final long serialVersionUID = 1L;
		});
		...
		WebMarkupContainerWithAssociatedMarkup fishDiv = new WebMarkupContainerWithAssociatedMarkup(
				"FishDiv");
		...
		feedForm.add(new AjaxButton("FeedFish") {
			...
			protected void onSubmit(AjaxRequestTarget target, Form<?> feedForm) {
				...
				SolarDecathlonApplication.getBackend().doCommand(system, null,
						command, newFeedAmount);

				System.out.println("Feeding " + selectedFeedAmnt
						+ " oz of feed.");
			}
		...
		feedForm.add(new AjaxButton("HarvestFish") {
			...
			protected void onSubmit(AjaxRequestTarget target, Form<?> feedForm) {
				...
				System.out.println("Fishnum " + selectedFishNum
						+ " fish.");
				IHaleSystem system = IHaleSystem.AQUAPONICS;
				...
				System.out.println("Harvesting " + selectedFishNum
						+ " fish.");
			}
