...
public class HVACData {
   ...
   * consumption in mind and the outside temperatures is really hot (i.e., temperatures during
   * summer season).
   **/
  ...
  /**
   * The efficient temperature to maintain the home at when occupants are home with energy
   * consumption in mind and the outside temperatures is really cold (i.e., temperatures during
   * winter season).
   **/
  ...
  /**
   * The current home temperature, defaulted to -1000 to imply it hasn't been initialized to a valid
   * value.
   **/
  private static int currentHomeTemp = -1000;

  /**
   * Used to determine a baseline home temperature that will be influenced by insulation values and
   * the current outside temperature to help determine rate of temperature in the home when the HVAC
   * system has been issued a command to increase or decrease the current temperature.
   **/
  ...
  /**
   * Modifies the state of the system. Resulting temperature units are in Celsius. Outside and home
   * temperatures are influenced by time of day and the corresponding current month's average high
   * and average low temperatures as reflected by temperature data gathered from 2010 for Washington
   * D.C. The coldest part of the day is defined as just before and during sunrise. The hottest part
   * of the day is defined to be at 3:00 PM or 15th hour of the day. HVAC system should maintain
   * temperatures approximately 78 in the summer and 68 in the winter, adjusted by 10 degrees F
   * higher or lower if occupants aren't home for energy usage efficiency stated by some HVAC
   * web-sites. Lacking the home insulation value (R-value), this model assumes a static 3 min/C
   * degree change for simplicity. Home temperature values will change over time to meet the desired
   * temperature when commanded. Otherwise the home will have its home temperature influenced purely
   * by the outside temperature and hour of the day.
   */
  public static void modifySystemState() {
    ...
    if (currentHour >= sunriseHour && currentHour <= hottestHourInDay) {
      currentOutsideTemp =
          (int) ((currentHour - sunriseHour) * degreeChangeFromSunriseToHighTempPt) + avgLowTemp;
    }
    ...
    // to the sunrise of the next day.
      ...
      if (currentHour < sunriseHour) {
        currentOutsideTemp =
            (int) (avgHighTemp - ((24 - hottestHourInDay + currentHour) * 
                degreeChangeFromHighTempPtToSunrise));
      }
      // The currentHour is still within the current day, after the hottest point in the day, but
      // before midnight.
      else {
        currentOutsideTemp =
            (int) (avgHighTemp - ((24 - currentHour) * degreeChangeFromHighTempPtToSunrise));
      }
    }
        ...
        if (desiredTemp > currentHomeTemp) {
          ...
          currentHomeTemp =
              ...
              // ((new Date().getTime()) - whenDesiredTempCommandIssued)
                  (currentTime.getTime() - whenDesiredTempCommandIssued) / (1000 * 60 * 
                      numMinOneDegreeCelChange));
        }
        // otherwise the trend is to cool down the room.
        else if (desiredTemp < currentHomeTemp) {
          currentHomeTemp =
              baseHomeTemp
                  - (int) ((currentTime.getTime() - whenDesiredTempCommandIssued) / (1000 * 60 * 
                      numMinOneDegreeCelChange));
        }
      }
    }
   ...
   * 
   * @param currentTime A Date object with the time of when we are interested if the occupants are
   * home. I.e., if we are interested to know if the occupants are at home at 1:00 PM on Saturdays,
   * the Date object will be Saturday with an hour time of 1:00 PM.
   ...
   * 
   * @param currentTime A Date object with the time of when we are interested in the occupants sleep
   * state. I.e., if we are interested to know if the occupants are sleeping at 6:00 AM the Date
   * object will be an arbitrary day with a time of 6:00 AM.
  ...
  /**
   * Returns the milliseconds since elapsed past January 1, 1970 00:00:00 GMT that a PUT request has
   * been sent to the HVAC system to maintain the home temperature at.
   * 
  ...
  /**
   * Appends HVAC state data at a specific timestamp snap-shot to the Document object passed to this
   * method.
   * 
